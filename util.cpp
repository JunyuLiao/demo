...
int show_to_user(point_set_t* P_raw, point_set_t* S, std::set<int> selected_dimensions, point_t* u){
    ...
    for (int j = 0; j < size; ++j) {
        printf("Point %d: ", j);
        for (int k = 0; k < m; ++k) {
            printf("%f ", S->points[j]->coord[k]);
        }
        printf("\n");
    }

    printf("Please choose the option you favor more:\n");
    printf("|%10s", "Options");
    for (int j = 0; j < m; ++j) {
    ...
    int maxIdx = -1;
    // scanf("%d", &maxIdx);
    // while the input is not in {0,1,2,...,size}
    while (maxIdx != 0 && maxIdx != 1 && maxIdx != 2 && maxIdx != -99){
        scanf("%d", &maxIdx);
    }
    return maxIdx==-99 ? -99 : maxIdx-1;
    // // construct u_selected
    // point_t* u_selected = alloc_point(m);
    // for (int j = 0; j < m; ++j) {
    //     u_selected->coord[j] = u->coord[*next(selected_dimensions.begin(), j)];
    // }
    // double max = 0;
    // int max_i = 0;
    // for (int j = 0; j < size; ++j) {
    //     double value = dot_prod(u_selected, S->points[j]);
    //     if (value > max) {
    //         max = value;
    //         max_i = j;
    //     }
